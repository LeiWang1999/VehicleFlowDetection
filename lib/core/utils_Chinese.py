...
from core.config_Chinese import cfg
...
def draw_bbox(image, bboxes, classes=read_class_names(cfg.YOLO.CLASSES), show_label=True):
    ...
    for i, bbox in enumerate(bboxes):
        ...
        if show_label:
            ...
            text_size = draw.textsize(bbox_mess, font)
            draw.text((c1[0], c1[1]-16), bbox_mess,  fill=(0, 0, 0), font=font)
            draw.rectangle((c1, c2))
            draw.rectangle(
                (c1[0], c1[1], c1[0] + text_size[0], c1[1] - text_size[1] - 3))
...
def postprocess_boxes(pred_bbox, org_img_shape, input_size, score_threshold):
    ...
    pred_xywh = pred_bbox[:, 0:4]       # 进行切片 行：全部，列：前四列  0 1 2 3 代表框的位置（四个坐标）
